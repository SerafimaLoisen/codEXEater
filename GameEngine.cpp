...
void GameEngine::update() {
    ...
    // ÂÀÆÍÎ: Äîáàâëÿåì äåáàã èíôîðìàöèþ î òåêóùåì ÷åêïîèíòå
    ...
    //if (bossMode) {
    //    // Áîññ ðåæèì - ÷àùå è ñëîæíåå
    //    /*updateBoss();
    //    checkBossCollisions();
    //    randomSpawnRate = 10 + (std::rand() % 15);*/
    //}
    //else {
    //    // Òóòîðèàë - ðåæå è ïðîùå
    ...
    // Ïðîâåðÿåì ñòîëêíîâåíèÿ ïóëü ñ èãðîêîì
    checkCollisions();

    // Îáðàáàòûâàåì ïàðèðîâàíèå
    handleParry();

    // Ïðîâåðÿåì óñëîâèå ñìåðòè (HP = 0)
    if (!player->isAlive()) {
...
void GameEngine::spawnBullet() {
    ...
    // Ð˜ÑÐ¿Ð¾Ð»ÑŒÐ·ÑƒÐµÐ¼ Ð»Ð¾ÐºÐ°Ð»ÑŒÐ½Ð¾ Ñ€Ð°ÑÑÑ‡Ð¸Ñ‚Ð°Ð½Ð½Ñ‹Ðµ Ñ€Ð°Ð·Ð¼ÐµÑ€Ñ‹ Ð¼Ð¸Ñ€Ð°
    auto& config = ConfigManager::getInstance();
    int worldWidth = config.getWorldWidth(currentLevel);
    int worldHeight = config.getWorldHeight(currentLevel);
    ...
    if (bossMode) {
        // Ð‘Ð¾ÑÑ Ñ€ÐµÐ¶Ð¸Ð¼ - Ð¿ÑƒÐ»Ð¸ Ð½Ð° Ñ€Ð°Ð·Ð½Ñ‹Ñ… Ð²Ñ‹ÑÐ¾Ñ‚Ð°Ñ…
        spawnY = 5 + (std::rand() % (worldHeight - 10));
    }
    else {
        // Ð¢ÑƒÑ‚Ð¾Ñ€Ð¸Ð°Ð» - Ð¿ÑƒÐ»Ð¸ Ñ‚Ð¾Ð»ÑŒÐºÐ¾ Ð½Ð° ÑÑ€ÐµÐ´Ð½Ð¸Ñ… Ð²Ñ‹ÑÐ¾Ñ‚Ð°Ñ…
        spawnY = 8 + (std::rand() % 5);
    }
...
void GameEngine::checkCollisions() {
    ...
    for (auto& bullet : projectiles) {
        ...
        if (player->checkCollision(*bullet)) {
            ...
            // Çàïîìèíàåì çäîðîâüå ÄÎ ïîëó÷åíèÿ óðîíà
            ...
            // ÈÌÅÍÍÎ ÇÄÅÑÜ ÍÓÆÍÎ ÒÅËÅÏÎÐÒÈÐÎÂÀÒÜÑß ÍÀ ×ÅÊÏÎÈÍÒ ÏÐÈ ËÞÁÎÌ ÓÐÎÍÅ
            if (currentCheckpoint) {
                Logger::Log("×ÅÊ 8");
                Logger::Log("Teleporting to checkpoint after taking damage! Current checkpoint: (" +
                    ...
                    std::to_string(currentCheckpoint->getY()) + ")");
                respawnAtCheckpoint();
                // Íå âûõîäèì èç öèêëà - ïðîäîëæàåì ïðîâåðÿòü äðóãèå ïóëè
            }
            else {
                Logger::Log("No active checkpoint to teleport to!");
            }

            // Åñëè èãðîê óìåð, íè÷åãî íå äåëàåì çäåñü - update() îáðàáîòàåò ñìåðòü
        }
    }
}
...
//
//    // ÐŸÑ€Ð¾Ð²ÐµÑ€ÑÐµÐ¼ Ð¿Ð¾Ð¿Ð°Ð´Ð°Ð½Ð¸Ðµ Ð¿ÑƒÐ»ÑŒ Ð¸Ð³Ñ€Ð¾ÐºÐ° Ð² Ð±Ð¾ÑÑÐ°
//    for (int i = projectiles.size() - 1; i >= 0; --i) {
...
//            boss->takeDamage(1);  // ÐšÐ°Ð¶Ð´Ð°Ñ Ð¿ÑƒÐ»Ñ Ð½Ð°Ð½Ð¾ÑÐ¸Ñ‚ 1 ÑƒÑ€Ð¾Ð½
//            projectiles[i]->setActive(false);
//        }
//    }
//
//    // ÐŸÑ€Ð¾Ð²ÐµÑ€ÑÐµÐ¼ ÑÑ‚Ð¾Ð»ÐºÐ½Ð¾Ð²ÐµÐ½Ð¸Ðµ Ð¸Ð³Ñ€Ð¾ÐºÐ° Ñ ÐºÐ¾Ñ€Ð½ÑÐ¼Ð¸ Ð±Ð¾ÑÑÐ°
//    for (auto& root : bossRoots) {
//        if (player->checkCollision(*root)) {
//            player->takeDamage(root->getDamage());
//            // ÐœÐ¾Ð¶Ð½Ð¾ ÑƒÐ´Ð°Ð»Ð¸Ñ‚ÑŒ ÐºÐ¾Ñ€ÐµÐ½ÑŒ Ð¿Ð¾ÑÐ»Ðµ Ð½Ð°Ð½ÐµÑÐµÐ½Ð¸Ñ ÑƒÑ€Ð¾Ð½Ð°
...
void GameEngine::createCheckpointsFromUIFrame() {
    ...
    for (int y = 0; y < uiFrame.size(); y++) {
        ...
        for (int x = 0; x < line.length(); x++) {
            ...
            // Èñïîëüçóåì ñèìâîë èç êîíôèãà äëÿ îáîçíà÷åíèÿ ÷åêïîèíòà
            if (currentChar == checkpointSymbol) {
                auto checkpoint = std::make_shared<Checkpoint>(x, y);
                checkpoints.push_back(checkpoint);

                // Óäàëÿåì ñèìâîë ÷åêïîèíòà èç UIFrame
...
void GameEngine::activateCheckpoint(std::shared_ptr<Checkpoint> checkpoint) {
    ...
    if (checkpoint) {
        ...
        if (!checkpoint->isActive()) {
            ...
            // Äåàêòèâèðóåì ÂÑÅ ïðåäûäóùèå ÷åêïîèíòû
...
void GameEngine::respawnAtLevelStart() {
    if (player) {
        ...
        player->setVelocityY(0);
        player->setOnGround(false);
        player->heal(player->getMaxHealth()); // Âîññòàíàâëèâàåì âñå HP
